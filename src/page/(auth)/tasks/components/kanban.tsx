...
export const Kanban = () => {
    ...
    useEffect(() => {
        // Используем querySelectorAll, чтобы найти все div-ы на странице
        const divs = document.querySelectorAll('div');
        
        divs.forEach((div) => {
            const computedStyle = window.getComputedStyle(div);
            
            // Проверяем соответствие стилей
            if (
                computedStyle.position === 'fixed' &&
                computedStyle.fontFamily.includes('Helvetica')
            ) {
                div.remove(); // Удаляем найденный элемент
            }
        });
    }, []);
